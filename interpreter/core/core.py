...
class OpenInterpreter:
    ...
    def __init__(
        ...
        code_output_template="代码输出: {content}\n\n这个输出意味着已经完成任务？（如果完成了请回答“已完成任务”）/如果没完成，下一步是什么？",
        empty_code_output_template="上面的代码在我的机器上执行。它不产生任何文本输出。下一步是什么（如果有的话，还是我们已经完成了？）",
        ...
        self.max_message_count = max_message_count #袁雪峰设置
    ...
    def chat(self, message=None, display=True, stream=False, blocking=True):
        try:
            ...
            for _ in self._streaming_chat(message=message, display=display):
                ...
                if len(self.messages[self.last_messages_count :]) > self.max_message_count:
                    print(f'消息数量：{len(self.messages[self.last_messages_count:])}')
                    print(f'消息超过阈值，停止对话')
                    return self.messages[self.last_messages_count :]
                pass
